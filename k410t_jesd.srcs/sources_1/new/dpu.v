...
//Event Flag
`define STARTFLAG {1'b1,15'h0000,1'b1,15'h0000}
`define ENDFLAG 32'b01010101010101010101010101010101

module dpu#(parameter EVENT_DEPTH = 1000,
		              START_OF_FRAME = 40
)
(
    input wire clk,
...
assign event_detA = (channelA < V_threshold);
assign event_detB = (channelB < V_threshold);
	    ...
	    if (event_detected) begin //write_preamble 
	       time_out <= time_out + 1;
            if (time_out>time_min) begin
                ...
                events[START_OF_FRAME] <= `STARTFLAG; //write enable once see flag
                //load in metadata
